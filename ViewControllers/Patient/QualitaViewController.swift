//
//  QualitaViewController.swift
//  IoRespiro2019
//
//  Created by sanganan on 22/12/2020.
//
...
class QualitaViewController: UIViewController, AirQualitaNewSec2CellDelegate {
    
    //variable
    var airQualityArr = [AirQuailtaModal]()
    var aqiArr : [CGFloat] = []
    var dateArr : [String] = []
    var foreAqiArr : [CGFloat] = []
    var foreDateArr : [String] = []
    var moreData = false
    var pageNumber = 1
    ...
    @IBOutlet weak var lineCharVw: LineChart!
    ...
    override func viewDidLoad() {
        ...
        commonNavigationBar(title: "Qualità dell’aria", controller: Constant.Controllers.patientQualita, color: UIColor.statusBarColor, shadowColor: .patientShadowColor, icon: "bellIcon", badgeCount: "")
...
extension QualitaViewController : UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        vw.bgView.backgroundColor = .clear
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0{
            ...
            //  cell.imgView.image = UIImage(named: "QualitaMedia")
            
            //            cell.outerView.backgroundColor = .patientThemeColor
            //            //Grafico AQI
            //
            ...
            cell.mainVw.backgroundColor = .patientThemeColor
        ...
        }else if indexPath.section == 4{ // forecast
            ...
            if foreChart == nil {
                cell.lineChartUI(data: self.foreAqiArr, xLabels: self.foreDateArr)
                foreChart = cell.chartView
            ...
            cell.aqiRingVw.isHidden = true
            cell.aqiImgWidthCons.constant = 0
            return cell
            
            
        }else if indexPath.section == 2  //AQI
        {
            let cell = tableView.dequeueReusableCell(withIdentifier: "AirQualitaSec2TableViewCell", for: indexPath) as! AirQualitaSec2TableViewCell
            ...
            let myString1 = "\(self.cityName)"
            let myAttribute1 = [ NSAttributedString.Key.foregroundColor: UIColor.white,NSAttributedString.Key.font: UIFont.appTitle]
            let myAttrString1 = NSAttributedString(string: myString1, attributes: myAttribute1)
            
            let myString = "(Stazione di \(self.stationNme))"
            let myAttribute = [ NSAttributedString.Key.foregroundColor: UIColor.white,NSAttributedString.Key.font: UIFont.profileListFont]
            ...
            cell.aqiRingVw.isHidden = false
            cell.aqiImgWidthCons.constant = 80
            return cell
        ...
        }else if indexPath.section == 3
        {
            ...
            cell.mainVw.roundCorners([.topLeft,.topRight], radius: 15)
            cell.mainVw.backgroundColor = .patientThemeColor
            cell.pollutantLbl.text = "Previsioni nella tua posizione corrente"
            cell.pollutantLbl.textColor = .white
            cell.pollutantLbl.font = .textFont
...
extension QualitaViewController : LineChartDelegate
{
    func didSelectDataPoint(_ x: CGFloat, yValues: [CGFloat]) {
        ...
        print(vc.pollutantArr,"pollutantArr data")
...
extension QualitaViewController
{
    ...
    func getAirDataFromServer(isleft : Bool)
    {
        ...
        Service.sharedInstance.postJSONRequest(Url: APIManager.APIs.getAirQualityDetails, modalName: AirQualitaResponseModal.self, parameter: request) { (modal, error) in
            ...
            if let forecast = mod.forecast
            {
                if forecast.count > 0 {
                    self.foreAqiArr.removeAll()
                    self.foreDateArr.removeAll()
                    for data in forecast
                    {
                        if let aqi = data.aqi , let date = data.day{
                            self.foreAqiArr.append(CGFloat(aqi) )
                            self.foreDateArr.append(date.convertDateFormatter())
                        }
                    }
                }
            }
...
extension QualitaViewController : AirQualityCellDelegate
{
    func leftBtnClicked()
    {
